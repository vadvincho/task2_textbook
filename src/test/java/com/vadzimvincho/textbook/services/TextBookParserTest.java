...
class TextBookParserTest {
    private static TextBookParser textBookParser;
    private static TextBook expectedTextBook;
    private static TextBook textBook;
    private static String expectedString;

    @BeforeAll
    static void init() {
        textBookParser = new TextBookParser();
        expectedTextBook = new TextBook();
        ...
        expectedTextBook.addTextComponent(paragraph1);
        expectedTextBook.addTextComponent(codeListing);
        expectedTextBook.addTextComponent(paragraph2);
    }

    @Test
    void parseTextbook(){
        textBook = textBookParser.parseTextbook(expectedString);
        List<TextBookComponent> textBookComponents = textBook.getTextComponent();
        Assertions.assertEquals(3,textBookComponents.size());
        Assertions.assertEquals(1,textBookComponents.get(0).getTextComponent().size());
        Assertions.assertEquals(5,textBookComponents.get(1).getTextComponent().size());
        Assertions.assertEquals(1,textBookComponents.get(2).getTextComponent().size());
    }

    @Test
    void getTextFromTextbook(){
        String getTextFromTextbook = expectedTextBook.getTextComponentToString();
        Assertions.assertEquals(expectedString,getTextFromTextbook);
    }
}